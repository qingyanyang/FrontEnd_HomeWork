...
const WeatherPage = () => {
  ...
  useEffect(() => {
    const getDefaultWeatherData = async () => {
      ...
      setDefaultWeatherInfo(validWeatherDataList);
    };
  ...
  useEffect(() => {
    const fetchSearchedWeatherData = async () => {
      if (searchedCityName) {
        const newWeatherInfo = await getWeatherDataByCityName(searchedCityName);
        if (newWeatherInfo) setSearchedCityWeatherData(newWeatherInfo);
      }
    };
  ...
  return (
    <CardsContainer $borderRadius={35} $padding={20}>
